<?php
  ...
  session_start();
    ...
    $stmt = $mysqli->prepare("INSERT INTO vpphotos (userid, filename, alt, privacy) VALUES (?, ?, ?, ?)");
			echo $mysqli->error;
		if(empty($privacy)){
			$privacy = 3;
		}
  ...
  function readprofilecolors(){
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
    $stmt = $mysqli->prepare("SELECT bg_color, txt_color FROM vpusers_profiles WHERE user_id=?");
	echo $mysqli->error;
	...
	if($stmt->fetch()){
		$_SESSION["bgColor"] = $bg_color;
		$_SESSION["txtColor"] = $txt_color;
	} else {
  ...
  function storeuserprofile($desc, $bgcol, $txtcol){
	$notice = "";
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
    $stmt = $mysqli->prepare("SELECT description, bg_color, txt_color FROM vpusers_profiles WHERE user_id=?");
	echo $mysqli->error;
	$stmt->bind_param("i", $_SESSION["userId"]);
	$stmt->bind_result($description, $bg_color, $txt_color);
	$stmt->execute();
	if($stmt->fetch()){
		//profiil juba olemas, uuendame
		$stmt->close();
		$stmt = $mysqli->prepare("UPDATE vpusers_profiles SET description=?, bg_color=?, txt_color=? WHERE user_id=?");
		echo $mysqli->error;
